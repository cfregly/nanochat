...
def _reset_kernels():
    # Ensure each test sees a clean registry
...
def test_clustered_attention_stub_fallback():
    config = _cfg(use_clustered_attention_kernel=True, allow_kernel_stub_fallback=True)
    model = GPT(config)
    x = torch.zeros((1, 4), dtype=torch.long)
    kv_cache = None
...
def test_persistent_decode_kernel_fallback():
    config = _cfg(use_persistent_decode_kernel=True, allow_kernel_stub_fallback=True)
    model = GPT(config)
    tok = _Tok()
    engine = Engine(model, tok, enable_batch_decode=False)
    ids = torch.zeros((1, 1), dtype=torch.long)
